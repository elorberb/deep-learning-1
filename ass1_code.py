...
def update_parameters(parameters: dict, grads: dict, learning_rate: float, l2: bool) -> dict:
    """
    Description:
    ...
    @param l2: bool weather to apply l2 norm
    ...
    for i in range(1, num_layers + 1):
        if l2:
            parameters[i][0] -= learning_rate * (grads[f"dW{i}"] + learning_rate * parameters[i][0])
        else:
...
def l_layer_model(X: np.array, Y: np.array, layers_dims: list, learning_rate: float, num_iterations: int,
                  batch_size: int, batch_norm: bool, l2: bool) -> tuple:
    """
    Description:
    ...
    while train_iter < num_iterations and not stop_early:
        ...
        while batch_begin < len(shuffled_ind) and not stop_early:
            ...
            if train_iter % 100 == 0:
                ...
                if l2:
                    for i in range(len(layers_dims)-1):
                        w = parameters[i+1][0]
                        cost += 0.5 * learning_rate * np.sum(w*w)
                costs.append(cost)
            ...
            parameters = update_parameters(parameters, grads, learning_rate, l2)
...
def create_nn(X_train_p, X_eval, y_train_p, y_eval, X_test, y_test, batch_norm, X_train, y_train, layers, learning_rate,
              number_iterations, batch_size, name, l2):
    """
    ...
    params, costs, costs_for_graph, number_run = l_layer_model(X_train, y_train, layers, learning_rate,
                                                               number_iterations, batch_size, batch_norm, l2=l2)
